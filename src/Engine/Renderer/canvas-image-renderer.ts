import { CustomImageDecoder } from "./image-decoder";
...
export class CanvasImageRenderer implements ICanvasImageRenderer {
  ...
  renderImage(assetKey: string, pos: Vec2, dir: Direction, anchor: Anchor) {
    const cacheKey = getImageCacheKey(assetKey, dir);

    let [cachedImage, cachedSize] = this._imagesCache.get(cacheKey) || [];

    if (!cachedImage || !cachedSize) {
      const [imageData, size] = this._imageDecoder.getImageData(assetKey, dir);
